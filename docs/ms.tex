...
\begin{equation}
p(\epsilon_{\ell}|\vec{g},\vec{\alpha})=p(\epsilon_{0}(\vec{g})|\vec{\alpha})\,|\frac{\partial\epsilon_{0}}{\partial\epsilon_{\ell}}|
\end{equation}
...
\begin{equation}
p(g_{1}....g_{n}|\vec{\alpha},Data)=p(g_{1}|\vec{\alpha},Data_{1})....p(g_{n}|\vec{\alpha},Data_{n})
\end{equation}
...
The total time needed to sample $g$ and $\vec{\alpha}$ of 64 sky patches with 8 galaxies in each patch is 
...
We also shows the histogram of g1 in a single sky patch in Figure 3. Not surprisingly it is close to a Gaussian distribution.
...
\end{figure}
\begin{figure}
\includegraphics[scale=0.3]{fig/alpha_beta}
\includegraphics[scale=0.3]{fig/stp10}

\caption{The upper panel show the MCMC samples of shape parameters of the Beta function model. The true(input) value is (2.8,2.8) as indicated by the two straight lines in the graphic. The lower panel shows the sample mean of the shape parameters of the step function model. The curve denotes the true shape distribution $P(|\epsilon_{0}|)$, which is Beta (2.8,2.8) in this case.}
\end{figure}
...
we have $m=-3.48\times10^{-4}\pm2.14\times10^{-3}$ and $c=(2.01\pm1.29)\times10^{-4}$. Figure 3 shows the result of bais test for the $g_{1}$ component.
...
Figure 5 shows the result of bias test for both mean ellipticity and posterior mean estimator.
...
Figure 7 shows the mean square error of the sample mean estimates in comparison with mean ellipticity estimator.
\begin{figure}
...
\end{figure}
We can see that our proposed method still works well under shape measurement error, even when we do not have the exact likelihood
of true lensed ellipticity. It means that this hierarchical Bayesian method does not only naturally cope with model fitting shape measurement 
method but is also compatible with ellipticity point estimators.
...
approach for weak lensing shear inference, with carefully calculated 
uncertainty estimates. This method intrinsically cope with high noise
