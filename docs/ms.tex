...
The second step is to infer the shear map from galaxies ellipticity.Usually the estimate of g  involve simple or weighted
means of measured galaxy ellipticities.
Finally,we can recover the projected mass profile of galaxies or dark energy by it’s shear map.
In this paper we set up a simple toy model in which 
...
How it's related to galaxy -galaxy lensing
In galaxy - galaxy lensing what people usually do is stack many lensing signals together and get the "average"
shear map,and then infers the mean mass profile of galaxy or galaxy cluster.
However with HB method we can calculate the posterior distribution of mass profile of each lens and propagate all the uncertainty
from to the posterior distributions.


This paper is orgnized as following: First we 
