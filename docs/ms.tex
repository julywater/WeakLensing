...
Cosmic shear is the mild distortion of distant galaxy images due to the bending of light by intervening matter.
Cosmic shear provides one of the most promising methods for constraining the nature of dark energy (Albrecht et al. 2006; Peacock&Schneider 2006). 
In galaxy--galaxy weaklensing  ,tangential shear distortions of background galaxies around foreground
galaxies, allows direct measurement of the galaxy–DM correlation
...
doing this we loose some information about how the ellipticity are distributed. Simple average point estimator can't 
...
in a hierachical way.Therefore in our simulation we set up an simple toy model in which we have the observed ellipticity as data,and the only measurement error we consider 
...
Our approach is to use an exceedingly flexiable model for intrinsic shape distribution,and assumes all the galaxies in different
patches abeys such an distribution.Then we can calculate the joint likelihood of both shpape distribution parameters and shear in differ
...
for \vec{g} in different patches simultaneously.
