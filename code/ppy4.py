...
def loglikefunc(X,E):
	gamma=np.array([0.0 for i in range(NP*N)])
	for i in range(0,NP):
...
f.close()
E=np.array([0.0 for i in range(NP*N)])
f=file("sheared.txt","w")
for i in range(0,NP):
	for j in range(0,N):
		E[i*NP+j]=shear(elipse[i*N+j],gamma[i])
...
for i in range(NP):
	for j in range(N):
		average[2*i]+=E[i*NP+N].real
		average[2*i+1]=E[i*NP+N].imag			
