...
epsilon=np.random.rayleigh(scale=0.25,size=N*NP)
if len(epsilon[epsilon>=1])>0:
	print('error')
...
P0=np.histogram(np.abs(E),bins=Nbin, range=((0,1)), normed=True)[0]
P0[P0==0]=0.10/N/NP
P0=np.log(P0/LIN)
...
Chain1=Gibbssampler(Chain[1999],postfunc,E,prosig,25000)
...
Q=Q/np.sum(Q)*10
P0=P0-math.log(np.sum(np.exp(P0)*(1.0/Nbin*np.linspace(0,Nbin-1,Nbin)+0.5/Nbin)))
P0=np.exp(P0)*(1.0/Nbin*np.linspace(0,Nbin-1,Nbin)+0.5/Nbin)
P0=P0/np.sum(P0)*10
a=1.0/Nbin*np.linspace(0,Nbin-1,Nbin)+0.5/Nbin
#plt.subplot(211)
plt.scatter(a,P)
#plt.scatter(a,P0,color='g')
